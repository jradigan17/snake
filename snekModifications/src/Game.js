...
class Game {
  ...
  handlePlayerInput(input, client) {
    ...
    users.push(client);
    users.forEach(connection => connection.write(`Everyone please welcome player ${name[name.length-1]} to the game. `));
    users.length > 1 ? users.forEach(connection => connection.write("We currently have " + users.length + " players.")) : users.forEach(connection => connection.write("We currently have " + users.length + " player."));
    usernames.push(name[name.length-1]);
  ...
  drawSnake(snake) {
    snake.segments.forEach((segment, i) => {
      this.ui.draw(segment, i === 0 ? 'red' : snake.color)
    })
    if (snake.message) {
  ...
  removeSnake(snake, index, message) {
    users.forEach(connection => connection.write(`Player ${usernames[index]} has left the game. `));
  ...
  checkPlayerHits() {
    ...
    for (let [i, snake] of this.snakes.entries()) {
      if (snake.hit(width, height)) {
        return this.removeSnake(snake, i, `* * *  G A M E  O V E R  * * * - You crashed!\n\t\t Your Score: ${snake["score"]}`)
      }

      if (SNAKE_COLLISIONS) {
        for (let [j, otherSnake] of this.snakes.entries()) {
          if (i !== j && snake.hitSnake(otherSnake)) {
            return this.removeSnake(snake, i, `* * *  G A M E  O V E R  * * * - You hit another snake!\n\t\t Your Score: ${snake["score"]}`)
          }
        }
      }
    }
